...
//       isAnimating = true;
//       didMoveDuringTurn = _mermaidCurrentPosition != index;

//       // if (_mermaidCurrentPosition == index) {
//       if (!didMoveDuringTurn) {
//         _showResult();
//       } else {
...
//     if (!didMoveDuringTurn) {
//     await _speakMermaidJump(_mermaidCurrentPosition);
//     }
...
class _ShellCountingGameState extends State<ShellCountingGame> {
  ...
  void _onShellTap(int index) {
    if (!_isReady || isProcessingAnswer || isAnimating) return;
  ...
  void _showResult() async {
    ...
    Future.delayed(const Duration(seconds: 2), () {
      _showPopup(
        context,
        _mermaidCurrentPosition == targetIndex ? "Congratulations!" : "Game Over",
        _mermaidCurrentPosition == targetIndex
            ? "You found the correct shell!"
            : "Wrong shell! Try again.",
        _mermaidCurrentPosition == targetIndex,
                        ...
                        child: GridView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return AspectRatio(
                              aspectRatio: 1.0,
                              child: GestureDetector(
                                onTap: isProcessingAnswer ? null : () => _onShellTap(cellNumber),
                                child: Stack(
                                  children: [
                                    Positioned.fill(
                                      child: SvgPicture.asset('assets/shell.svg'),
